...
def set_up_sim_directories(anatomy, set_type, geo_name, flow_name):
    ...
    dir = f"/scratch/users/nrubio/synthetic_vessels/{anatomy}/{set_type}/{geo_name}"
    results_dir = f"/scratch/users/nrubio/synthetic_vessels_reduced_results/{anatomy}/{set_type}/{geo_name}/"
    if os.path.exists(results_dir) == False:
        os.mkdir(results_dir)
    if os.path.exists(f"/scratch/users/nrubio/synthetic_vessels/{anatomy}/{geo_name}/{flow_name}"):
        os.system(f"rm -r /scratch/users/nrubio/synthetic_vessels/{anatomy}/{geo_name}/{flow_name}")
    os.system(f"mkdir /scratch/users/nrubio/synthetic_vessels/{anatomy}/{geo_name}/{flow_name}")
    return
...
def check_for_centerline(anatomy, set_type, geo_name):
    try:
        ...
        os.system(f"rm -r /scratch/users/nrubio/synthetic_vessels/{anatomy}/{geo_name}")
        return False
    return True
...
def load_params_dict(anatomy, set_type, geo_name):
    params_dict = load_dict(f"/scratch/users/nrubio/synthetic_vessels/{anatomy}/{geo_name}/junction_params_dict")
    dir = f"/scratch/users/nrubio/synthetic_vessels/{anatomy}"
